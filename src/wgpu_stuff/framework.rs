...
impl SurfaceWrapper {
    ...
    fn pre_adapter(&mut self, _instance: &Instance, _window: Arc<Window>) {}
...
impl ExampleContext {
    ...
    async fn init_async<E: Example>(surface: &mut SurfaceWrapper, window: Arc<Window>) -> Self {
        ...
        let adapter = wgpu::util
            ::initialize_adapter_from_env_or_default(&instance, surface.get()).await
            ...
            match event {
                ...
                Event::WindowEvent { event, .. } =>
                    match event {
                        ...
                        WindowEvent::KeyboardInput {
                            event: KeyEvent { logical_key: Key::Character(s), .. },
                            ..
                        } if s == "r" => {
